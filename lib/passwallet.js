...
var registerDevice = function (req, res) {
	...
	if ((req.get('x-forwarded-proto') && 'https' !== req.get('x-forwarded-proto')) || !req.secure) {
		return send(426);  // Upgrade Required (ie. switch to https://)
	}
	...
	async.series([
		...
		if (err) { // Note. Not always strictly an error but convenient construct
			...
			return send(201) // Created: Registration succeeded
		}
	});
};
